...
describe("AIM-272 deterministic actor LOD", () => {
  ...
  it("updates near mixers every frame, deterministically distributes a crowd across mid phases, and freezes proxy bands", () => {
    ...
    const schedule = (identity: string) => Array.from({ length: 12 }, (_, tick) => shouldUpdateActorAnimation(tick, identity, "mid"));
    const identities = Array.from({ length: 12 }, (_, index) => `player:${index + 1}`);
    const schedules = identities.map(schedule);
    for (const value of schedules) expect(value.filter(Boolean)).toHaveLength(4);
    expect(schedule("player:7")).toEqual(schedule("player:7"));
    const activePhases = schedules.map(value => value.findIndex(Boolean));
    expect(new Set(activePhases).size).toBeGreaterThan(1);
