...
export function persistConfirmedPlayLaunch(value: unknown): boolean {
  ...
  try {
    sessionStorage.setItem(AURION_PLAY_LAUNCH_KEY, JSON.stringify(value));
    return true;
  } catch { return false; }
}

function readLaunch(): PlayLaunch | null {
  try {
...
export default function AurionPlayRoute() {
  ...
  useEffect(() => {
    if (!launch) return;
    // Child effects have mounted by this point; microtask keeps the original
    // server-confirmed launch payload intact while avoiding a second HTTP action.
