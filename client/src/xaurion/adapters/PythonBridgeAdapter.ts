...
export class PythonBridgeAdapter extends GameAdapter {
  ...
  constructor(private readonly simulation: DeterministicSimulation) { super('PythonEngineBridgeAdapter'); }
  public logEvent(type: string, payload: any) {
    this.eventLog.push({ type, timestamp: this.simulation.elapsedMilliseconds, payload });
  ...
  public exportTelemetryJson(): string { return JSON.stringify({ engine: 'Aurion-MMORPG-Core', version: '2.0.0', timestamp: this.simulation.elapsedMilliseconds, eventsCount: this.eventLog.length, events: this.eventLog }, null, 2); }
}
