...
import { BookOpen, Box, LogOut, ShieldCheck, Sparkles, Swords, UserRound, UsersRound } from "lucide-react";
...
/**
 * Aurion account portal: read-only projections of persisted game data.
 * The only write exposed here is account logout. Character/gameplay mutations
 * belong to AX1/WASD and are intentionally absent from this surface.
 */
export default function Account() {
  const { user, isAuthenticated, loading, logout } = useAuth();
  const options = { enabled: isAuthenticated, retry: false, refetchOnWindowFocus: false } as const;
  const player = trpc.player.me.useQuery(undefined, options);
  const ui = trpc.player.ui.useQuery(undefined, options);
  const group = trpc.groups.read.useQuery(undefined, options);
  const crafting = trpc.crafting.read.useQuery(undefined, options);
  const gateways = trpc.gateway.listSessions.useQuery(undefined, options);
  ...
  const training = useMemo(() => {
    ...
    for (const row of rows) {
      ...
      if (!prior) sessions.set(row.session_id, { rows: 1, latest: row.timestamp_epoch, lastSampleId: row.sample_id });
        ...
        prior.rows += 1;
        if (row.timestamp_epoch >= prior.latest) { prior.latest = row.timestamp_epoch; prior.lastSampleId = row.sample_id; }
      }
    }
  ...
  return <main className="min-h-screen bg-[#061317] text-slate-100">
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-5 py-8">
      <section className="rounded-3xl border border-cyan-300/20 bg-[#0b2024]/90 p-6">
