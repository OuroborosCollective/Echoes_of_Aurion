...
import { glbImportReceiptSchema, glbRuntimeCatalogSchema, type GlbEquipmentSlot, type GlbImportPurpose, type GlbImportReceipt, type GlbRuntimeCatalog } from "@shared/glbImportContract";
...
export default function GlbUpload() {
  ...
  const uploadOne = async (file: File, chosenName: string): Promise<SmartUploadResult> => {
    const response = await fetch("/api/admin/glb-smart-upload", {
      ...
      body: JSON.stringify({ displayName: chosenName, fileName: file.name, purpose, contentBase64: await readFileAsBase64(file) }),
    });
    ...
    if (purpose !== "auto" && (receipt.data.targetKey !== null || receipt.data.status !== "catalog")) throw new Error("Der Server hat die katalog-only Grenze des gewählten Zwecks nicht bestätigt.");
    if (purpose === "npc-fallback" && receipt.data.assetType !== "character") throw new Error("Der Server hat keinen Charakter-NPC-Fallback bestätigt.");
    if (purpose === "player-public" && receipt.data.assetType !== "character") throw new Error("Der Server hat keinen öffentlichen Spielercharakter bestätigt.");
    if (purpose === "world-environment" && (receipt.data.assetType !== "arena" || payload.classification.worldFamily !== "environment")) throw new Error("Der Server hat kein Umgebungsobjekt bestätigt.");
    if (purpose === "world-nature" && (receipt.data.assetType !== "arena" || payload.classification.worldFamily !== "nature")) throw new Error("Der Server hat kein Naturmodell bestätigt.");
    if (purpose === "equipment" && (!payload.classification.equipmentSlot || !["weapon", "armor"].includes(receipt.data.assetType))) throw new Error("Der Server hat keinen tragbaren Ausrüstungsslot bestätigt.");
    return payload;
  };
  ...
  return <DashboardLayout><div data-testid="glb-upload-scroll-region" tabIndex={0} className="h-[calc(100dvh-5.5rem)] overflow-y-auto overscroll-y-contain touch-pan-y [scrollbar-gutter:stable] md:h-[calc(100dvh-2rem)] bg-[radial-gradient(circle_at_top_right,rgba(45,226,207,.12),transparent_38%),#06131a] p-3 text-slate-100 sm:p-6">
    <div className="mx-auto max-w-4xl space-y-5 pb-10">
      ...
      <Card className="border-cyan-200/15 bg-slate-950/75"><CardHeader><CardTitle className="flex items-center gap-2 text-amber-100"><Upload className="h-5 w-5" />GLB aufnehmen</CardTitle><CardDescription>Bis zu {MAX_GLB_BATCH_FILES} Dateien pro Durchlauf, jeweils maximal 24 MiB. Unklare Modelle werden einzeln abgelehnt.</CardDescription></CardHeader><CardContent className="space-y-4">
        <div className="space-y-2"><Label htmlFor="smartGlbPurpose">Kategorie / Verwendungszweck</Label><select id="smartGlbPurpose" value={purpose} disabled={busy} onChange={event => setPurpose(event.target.value as GlbImportPurpose)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background">{(Object.keys(purposeLabels) as GlbImportPurpose[]).map(value => <option key={value} value={value}>{purposeLabels[value]}</option>)}</select><p className="text-xs leading-5 text-slate-400">{purposeDescriptions[purpose]}</p></div>
        ...
        <label htmlFor="smartGlbFile" className="flex min-h-44 cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-cyan-300/30 bg-cyan-400/[.035] p-6 text-center transition-colors hover:bg-cyan-400/[.06]" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); void uploadFiles(Array.from(event.dataTransfer.files)); }}><FileBox className="h-8 w-8 text-cyan-300" /><div><p className="font-medium text-amber-50">GLBs hier ablegen oder gemeinsam auswählen</p><p className="mt-1 text-xs text-slate-400">Dateiname und GLB-Inhalt werden gemeinsam klassifiziert; der Server bestätigt jeden Zweck separat.</p></div><Input id="smartGlbFile" type="file" multiple accept=".glb,model/gltf-binary" disabled={busy || !catalog || Boolean(storageError)} className="max-w-sm" onChange={event => { void uploadFiles(Array.from(event.target.files ?? [])); event.currentTarget.value = ""; }} /></label>
      ...
      {catalog && catalog.entries.length > 0 && <Card className="border-cyan-200/15 bg-slate-950/75"><CardHeader><CardTitle>Veröffentlichte Modelle</CardTitle><CardDescription>Zweckgebundene Modelle bleiben katalog-only; Runtime-Projektionen lesen nur ihren eigenen Zweck.</CardDescription></CardHeader><CardContent className="space-y-3">{catalog.entries.map(entry => <div key={`${entry.assetId}:${entry.targetKey}`} className="flex flex-wrap items-center justify-between gap-2 border-b border-cyan-200/10 pb-3 text-sm"><div><b>{entry.displayName}</b><p className="text-xs text-slate-400">{entry.assetType} · {purposeLabels[entry.purpose]} · {entry.equipmentSlot ? `Slot ${entry.equipmentSlot}` : entry.subcategory ?? entry.targetKey ?? "Nur Katalog"}</p></div><a className="text-cyan-200 underline" href={entry.storageUrl} target="_blank" rel="noreferrer">GLB öffnen</a></div>)}</CardContent></Card>}
    </div>
  </div></DashboardLayout>;
}
