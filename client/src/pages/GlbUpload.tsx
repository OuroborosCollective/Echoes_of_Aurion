...
import { glbImportReceiptSchema, glbRuntimeCatalogSchema, type GlbImportPurpose, type GlbImportReceipt, type GlbRuntimeCatalog } from "@shared/glbImportContract";
...
export default function GlbUpload() {
  ...
  const uploadOne = async (file: File, chosenName: string): Promise<SmartUploadResult> => {
    ...
    if (payload.purpose !== purpose) throw new Error("Der Server hat einen abweichenden Verwendungszweck bestätigt.");
    if (purpose === "npc-fallback" && (receipt.data.assetType !== "character" || receipt.data.targetKey !== null || receipt.data.status !== "catalog")) {
      throw new Error("Der Server hat den NPC-Fallback nicht als unzugeordnetes Charaktermodell bestätigt.");
    }
    return payload;
  };
  ...
  const replaceTarget = async (result: SmartUploadResult) => {
    ...
    if (result.purpose === "npc-fallback" || !receipt.targetKey) { setError("NPC-Fallbacks besitzen absichtlich kein ersetzbares Spielziel."); return; }
    try {
  ...
  return <DashboardLayout><div className="min-h-full bg-[radial-gradient(circle_at_top_right,rgba(45,226,207,.12),transparent_38%),#06131a] p-3 text-slate-100 sm:p-6">
    <div className="mx-auto max-w-4xl space-y-5">
      <header><p className="text-xs tracking-[.24em] text-cyan-300">AURION // ASSET INTAKE</p><h1 className="mt-2 text-3xl font-semibold text-amber-100">GLB automatisch einsortieren</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Modelle hochladen, erkennen und im Spiel verwenden. Geprüfte Admin-Uploads werden veröffentlicht und nur innerhalb des unten gewählten Zwecks verwendet. Ist ein normales Spielziel belegt, entscheidest du über den Austausch.</p></header>
      <p className="text-sm text-muted-foreground">NPC-Fallbacks sind eine reine Darstellungsfläche: Sie dürfen ausschließlich als Charaktere katalogisiert werden, erhalten niemals ein Spielerziel und verändern keine Quest-, Händler-, Kampf- oder Weltlogik.</p>
      ...
      <Card className="border-cyan-200/15 bg-slate-950/75">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2"><Label htmlFor="smartGlbPurpose">Verwendungszweck</Label><select id="smartGlbPurpose" value={purpose} disabled={busy} onChange={event => setPurpose(event.target.value as GlbImportPurpose)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background"><option value="auto">Automatisch zuordnen</option><option value="npc-fallback">Nur NPC-Fallback · niemals Spieler</option></select><p className="text-xs text-slate-400">„NPC-Fallback“ katalogisiert nur serverseitig bestätigte Charaktermodelle. Es wird kein Target ersetzt oder neu belegt.</p></div>
