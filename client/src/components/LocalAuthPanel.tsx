...
export default function LocalAuthPanel() {
  ...
  function startOidcLogin() {
    // Keep the redirect control focusable while busy. aria-disabled communicates
    // the bounded loading state; this guard prevents duplicate submissions.
    ...
    setOidcLoading(true);
    navigateBrowser(aurionApiUrl("/api/oauth/start"));
  }
    ...
    <AnimatePresence>
      {open && (
        <motion.div 
          ...
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center overflow-y-auto bg-[#061317]/90 p-4 backdrop-blur-sm" 
          role="dialog" 
          aria-modal="true" 
          aria-labelledby="aurion-auth-title"
        >
          <div className="w-full max-w-md my-auto flex-shrink-0">
            <motion.form 
              ...
              onSubmit={submit} 
              className="w-full rounded-2xl border border-cyan-300/35 bg-[#0b2024] p-5 text-slate-100 shadow-2xl shadow-cyan-950/50 sm:p-7 gate-panel"
            >
            <header className="mb-6 flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setOpen(false)} className="grid size-12 place-items-center rounded-lg border border-slate-400/35 text-slate-200 transition hover:bg-white/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-300" aria-label="Anmeldedialog schließen" title="Anmeldedialog schließen"><X size={20} /></button>
            </header>
            ...
            <button type="button" onClick={startOidcLogin} aria-disabled={oidcLoading} aria-busy={oidcLoading} title={oidcLoading ? "Wird umgeleitet..." : undefined} className="group mb-4 min-h-12 w-full rounded-lg border border-cyan-300/55 bg-cyan-300/10 px-4 text-sm font-bold text-cyan-100 transition-all hover:bg-cyan-300/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 hover:-translate-y-0.5 active:scale-95 aria-disabled:cursor-not-allowed aria-disabled:opacity-50 aria-disabled:hover:translate-y-0 aria-disabled:active:scale-100"><ShieldCheck className="mr-2 inline size-4 transition-transform group-hover:scale-110" />{oidcLoading ? "WIRD GELADEN..." : "Mit FusionAuth anmelden"}</button>
