...
export default function CommunityOverlay({
  ...
  const craftItem = trpc.crafting.craft.useMutation({ onSuccess: async result => { setMessage(result.applied ? "Dein magischer Aurionspeer ist fertig. Du hast 6 Handwerks-EP erhalten." : "Dieser Speer wurde bereits veredelt."); await Promise.all([utils.crafting.read.invalidate(), utils.market.inventory.invalidate(), utils.player.me.invalidate()]); }, onError: error => reportRuntimeFailure("Die Speerveredelung", error) });
  const collectCraftingBonus = trpc.crafting.materializeBonus.useMutation({ onSuccess: async result => { setMessage(result.applied ? `${result.outputs.length} Bonusgegenstände wurden ins Inventar übernommen.` : "Diese Bonusgegenstände wurden bereits übernommen."); await Promise.all([utils.crafting.read.invalidate(), utils.market.inventory.invalidate()]); }, onError: error => reportRuntimeFailure("Die Bonusausgabe", error) });
  ...
  const overlay = <aside className="community-overlay" aria-label="Aurion Gemeinschaft" data-mobile-menu-open={mobileMenuOpen} data-opened-from-world={openedFromWorld && panel !== null}>
    ...
    {panel && <section className={`community-panel ${panel}-panel`} aria-live="polite">
      ...
      {panel === "crafting" && <div className="community-crafting">
        {crafting.isLoading && <p className="community-empty">Die Schmiede wird geladen…</p>}
        ...
        <div className="community-intro"><Hammer size={18} /><div><b>Aurionspeer tempern</b><p>Veredle einen gewöhnlichen Aurionspeer zu einem magischen Speer. Du benötigst einen eigenen Speer und erhältst 6 Handwerks-EP.</p></div></div>
        <div className="community-section-heading"><span>DEINE MATERIALIEN</span><i /></div>
        <div className="market-inventory">{marketInventory.data?.filter(item => item.baseItemKey === "aurion_spear" && item.quality === "normal").map(item => <article key={item.id} className="market-item"><header><b>{item.baseItemKey.replaceAll("_", " ")}</b><span className={`market-quality ${item.quality}`}>{item.quality}</span></header><p>Gegenstandsstufe {item.itemLevel} · Wird beim Tempern zu einem magischen Speer.</p><button type="button" className="crafting-action" onClick={() => craftItem.mutate({ recipeKey: "temper_aurion_spear", inputItemId: item.id })} disabled={craftItem.isPending}><Hammer size={15} />{craftItem.isPending ? "Wird geprüft…" : "Speer tempern"}</button></article>)}{!marketInventory.isLoading && !marketInventory.data?.some(item => item.baseItemKey === "aurion_spear" && item.quality === "normal") && <p className="community-empty">Du hast noch keinen gewöhnlichen Aurionspeer. Du kannst ihn auf Expeditionen erbeuten.</p>}</div>
        <div className="community-section-heading"><span>DEINE WERKSTÜCKE</span><i /></div>
        <div className="market-inventory">{crafting.data?.outputs.map(item => <article key={item.id} className="market-item"><header><b>{item.baseItemKey.replaceAll("_", " ")}</b><span className={`market-quality ${item.quality}`}>{item.quality}</span></header><p>Gegenstandsstufe {item.itemLevel} · {item.affixes.map(affix => `${affix.slot}: ${affix.key}`).join(" · ") || "Keine Affixe"}</p></article>)}{!crafting.isLoading && !crafting.data?.outputs.length && <p className="community-empty">Du hast hier noch keinen Gegenstand hergestellt.</p>}</div>
        <div className="community-section-heading"><span>HANDWERK</span><i /></div>
        <p className="community-empty">{crafting.data ? `${crafting.data.progression.progression.totalXpExact} Handwerks-EP · Stufe ${crafting.data.progression.progression.levelExact} · ${crafting.data.receipts.length} Herstellung${crafting.data.receipts.length === 1 ? "" : "en"}` : "Noch keine Handwerkserfahrung."}</p>
        {crafting.data?.scopedMastery && <div aria-label="Bestätigte Schmiedemeisterschaft">{crafting.data.scopedMastery.map(mastery => <p className="community-empty" key={`${mastery.key.scopeType}:${mastery.key.scopeId}`}>{mastery.key.scopeType === "profession" ? "Schmiedekunst" : mastery.key.scopeType === "recipe" ? "Speerveredelung" : "Aurionspeer"}: Stufe {mastery.progression.levelExact} · {mastery.progression.totalXpExact} XP · {mastery.lifetimeUsesExact} Ausführungen</p>)}</div>}
        {crafting.data?.bonusOutputs?.map(batch => <article className="market-item" key={batch.professionReceiptId}><p>{batch.remainingQuantityExact} bestätigte Bonusgegenstände stehen bereit.</p><button type="button" disabled={collectCraftingBonus.isPending} onClick={() => collectCraftingBonus.mutate({ receiptId: batch.receiptId, expectedOutputIndexExact: batch.nextOutputIndexExact, count: Math.min(10, Number(batch.remainingQuantityExact)) })}>{collectCraftingBonus.isPending ? "Wird übernommen…" : "Bonus ins Inventar übernehmen"}</button></article>)}
      </div>}
      ...
      {panel === "assets" && <div className="community-assets">
        ...
        <div className="asset-catalog starter-asset-catalog">{starterCharacters.map(character => <article key={character.id}><div><b>{character.name}</b><span>standard · character · riggt + animiert</span><p>{character.role} · {character.detail}</p></div><div className="asset-catalog-actions">{character.assetPath ? <a href={character.assetPath} target="_blank" rel="noreferrer">Modell ansehen</a> : <span className="text-xs text-slate-400">Prozedurales Modell</span>}{isAuthenticated && <button type="button" disabled={starterCharacterId === character.id} onClick={() => onStarterCharacterSelected(character)}>{starterCharacterId === character.id ? "Aktiv" : "Als Charakter wählen"}</button>}</div></article>)}</div>
