  ...
  const overlay = <aside className="community-overlay" aria-label="Aurion Gemeinschaft" data-mobile-menu-open={mobileMenuOpen} data-opened-from-world={openedFromWorld && panel !== null}>
    ...
    <AnimatePresence mode="wait">
      {panel && <motion.section 
        ...
        aria-live="polite"
      >
        <header className="community-panel-header"><div><p>AURION // COMMUNITY</p><h2>{panel === "chat" ? "Signalraum" : panel === "forum" ? "Sternwartenforum" : panel === "events" ? "Community-Events" : panel === "guild" ? "Gildenzugehörigkeit" : "Öffentlicher Asset-Katalog"}</h2></div><button type="button" onClick={close} aria-label="Community-Konsole schließen" title="Community-Konsole schließen"><X size={18}/></button></header>
