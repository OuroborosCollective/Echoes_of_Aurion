  ...
  async load(): Promise<void> {
    ...
    try {
      const spider = await loadCreature(
        this.scene,
        STARTER_CHARACTER_ASSETS.spider.url,
        "starter-spider-visual",
        this.parent,
        1.55,
      );
      const beastLods: LoadedCreature[] = [];
      for (let index = 0; index < STARTER_CHARACTER_ASSETS.beast.lodUrls.length; index += 1) {
        beastLods.push(await loadCreature(
          this.scene,
          STARTER_CHARACTER_ASSETS.beast.lodUrls[index]!,
