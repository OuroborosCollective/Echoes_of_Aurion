...
export const appRouter = router({
  ...
  gameplay: router({
    ...
    acceptQuest: protectedProcedure.input(z.object({ questKey: z.enum(["astral_call", "archive_of_echoes", "ember_key", "starfall_resonance", "sunwatch_vanguard", "clockwork_core"]) })).mutation(({ ctx, input }) => db.acceptGameplayQuest({ userId: ctx.user.id, questKey: input.questKey as QuestKey })),
    completeQuest: protectedProcedure.input(z.object({ questKey: z.enum(["astral_call", "archive_of_echoes", "ember_key", "starfall_resonance", "sunwatch_vanguard", "clockwork_core"]), giver: z.enum(["Lyra", "Orun"]) })).mutation(async ({ ctx, input }) => {
    ...
    requestQuestActionFromDialogue: protectedProcedure.input(z.object({
      ...
      questKey: z.enum(["astral_call", "archive_of_echoes", "ember_key", "starfall_resonance", "sunwatch_vanguard", "clockwork_core"]),
