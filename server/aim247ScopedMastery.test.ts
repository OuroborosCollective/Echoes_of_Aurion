...
import { craftingMasteryKeys, classlessWeaponMasteryKey, legacyDisciplineMasteryKey, livingWorldSocialMasteryKeys } from "./scopedMasteryAdapters";
import { SCOPED_MASTERY_RULESET_VERSION, canonicalScopedMasteryKey, evaluateMasteryEligibility, masteryKeys, resolveCoupledMasteries, resolveScopedMastery, scopedMasteryKey, type ScopedMasteryEvent } from "./scopedMasteryProtocol";

function event(key: ReturnType<typeof scopedMasteryKey>, index: number, amountExact = "10", overrides: Partial<ScopedMasteryEvent> = {}): ScopedMasteryEvent {
  return {
...
describe("AIM-247 scoped infinite mastery kernel", () => {
  ...
  it("continues exactly above Number.MAX_SAFE_INTEGER without a level cap or overflow authority", () => {
    ...
    const current = {
      progression: { totalXpExact: "9007199254740993000", levelExact: "1000000", xpIntoLevelExact: "0", xpForNextLevelExact: "0", totalXp: Number.MAX_SAFE_INTEGER, level: 1_000_000, numberProjectionExact: false },
    ...
    const state = resolveScopedMastery({ actorId: "user:42", key, current, events: [event(key, 1, "1", { qualityDeltaExact: "1", contextMetricsExact: { successful_crafts: "1" } })] });
  ...
  it("keeps profession, recipe and item mastery separate but coupled to one confirmed craft receipt", () => {
    ...
    const events = [profession, recipe, item].map((key, index) => event(key!, index + 1, index === 0 ? "5" : index === 1 ? "8" : "3", { receiptId: "craft_receipt_1", idempotencyKey: `craft_scope_${index}` }));
    const states = resolveCoupledMasteries({ actorId: "user:42", keys: [profession!, recipe!, item!], events });
    expect(states.map(state => canonicalScopedMasteryKey(state.key))).toEqual(["v1:item:oak_chest", "v1:profession:carpentry", "v1:recipe:wooden_chest"]);
  ...
  it("maps classless weapons, social relations and legacy disciplines into the same scoped engine", () => {
    ...
    expect(livingWorldSocialMasteryKeys("negotiation", "lyra").map(canonicalScopedMasteryKey)).toEqual(["v1:social:negotiation", "v1:npc_relation:lyra"]);
  ...
  it("deduplicates idempotency keys and produces deterministic state hashes", () => {
