...
import type { NpcGoal, NpcNeedKey, NpcNeedState } from "./wasdAurionProtocol";
...
function freezePlan(plan: z.infer<typeof planSchema>): NpcLifePlan {
  return Object.freeze({ ...plan, steps: Object.freeze(plan.steps.map(step => Object.freeze({ ...step }))) });
}
function freezeState(parsed: z.infer<typeof stateSchema>): NpcLifeState {
  return Object.freeze({ ...parsed, personality: Object.freeze({ ...parsed.personality }), relationships: Object.freeze(parsed.relationships.map(value => Object.freeze({ ...value }))), plan: freezePlan(parsed.plan), ...(parsed.economy ? { economy: Object.freeze({ ...parsed.economy }) } : {}) });
}
...
export function resolveNpcLife(input: Readonly<{
  ...
  const opportunities = input.opportunities.map(value => npcLifeOpportunitySchema.parse(value)).slice().sort((a, b) => compareText(a.sourceReceiptId, b.sourceReceiptId) || compareText(a.id, b.id));
