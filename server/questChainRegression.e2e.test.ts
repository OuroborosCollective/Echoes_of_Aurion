...
describeWithDatabase("quest chain regression E2E", () => {
  ...
  it("preserves ordered quest gates, NPC-bound turn-ins, deferred rewards, Ember Key, Starfall, Sunwatch and dungeon access", async () => {
    ...
    expect(completed.profile).toMatchObject({ totalXp: 1202, aurionPoints: 190, seasonPoints: 190, victories: 4 });
    ...
    expect((await getGameplayProgress(QUEST_CHAIN_REGRESSION_USER_ID)).profile).toMatchObject({ totalXp: 1202, aurionPoints: 190, seasonPoints: 190, victories: 4 });
    await expect(completeGameplayQuest({ userId: QUEST_CHAIN_REGRESSION_USER_ID, questKey: "clockwork_core", giver: "Lyra" })).rejects.toThrow("Dieser Questgeber kann den Auftrag nicht abschließen.");
    const afterClockwork = await completeGameplayQuest({ userId: QUEST_CHAIN_REGRESSION_USER_ID, questKey: "clockwork_core", giver: "Orun" });
    expect(afterClockwork.profile).toMatchObject({ totalXp: 1852, aurionPoints: 290, seasonPoints: 290, victories: 5 });
    expect(afterClockwork.quests.map(quest => [quest.key, quest.state, quest.readyToTurnIn])).toEqual([
      ["astral_call", "completed", false],
      ["archive_of_echoes", "completed", false],
      ["ember_key", "completed", false],
      ["starfall_resonance", "completed", false],
    ...
    const fullyCompleted = await completeGameplayQuest({ userId: QUEST_CHAIN_REGRESSION_USER_ID, questKey: "sunwatch_vanguard", giver: "Orun" });
    expect(fullyCompleted.profile).toMatchObject({ totalXp: 2502, aurionPoints: 390, seasonPoints: 390, victories: 6 });
