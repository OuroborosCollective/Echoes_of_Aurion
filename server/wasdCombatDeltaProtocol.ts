import { createARESeed, SeededARERng, type ARERng, WASD_GAMEPLAY_SOURCE_REVISION } from "./wasdAREDeterminism";

/**
 * Gameplay truth is pinned to the canonical WASD source, not invented by Aurion.
 * Source: OuroborosCollective/Wasd@328240450d33490637f8cc4ae87d3fbeecca27c9
 * server/src/modules/combat/CombatDeltaResolver.ts
 */
...
type NormalizedCombatDeltaContext = { tick: number; sequence: number; weaponBonus: number; entropy?: number };
...
export function resolveCombatDelta(action: CombatDeltaAction, attacker: CombatDeltaEntityView, defender: CombatDeltaEntityView, context: CombatDeltaContext): CombatDelta {
  const normalizedContext: NormalizedCombatDeltaContext = { tick: safeInteger(context.tick, 0), sequence: safeInteger(context.sequence, 0), weaponBonus: safeInteger(context.weaponBonus, 0), entropy: context.entropy !== undefined ? context.entropy : undefined };
  ...
  const hit = (normalizedContext.entropy !== undefined ? normalizedContext.entropy : rng.nextFloat()) <= calculateCombatHitChance(attacker, defender);
