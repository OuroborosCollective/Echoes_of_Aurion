...
export function encounterSessionIdentity(userId: number, ordinal: number, encounterKey: string): string {
  if (!Number.isSafeInteger(userId) || userId < 1 || !Number.isSafeInteger(ordinal) || ordinal < 1 || ordinal > 2_147_483_647 || !["asterion", "archive", "solarium", "cinder_vault", "starfall_crater"].includes(encounterKey)) throw new Error("INVALID_ENCOUNTER_IDENTITY_CONTEXT");
