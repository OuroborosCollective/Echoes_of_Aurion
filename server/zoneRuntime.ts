...
import {
  ...
  type AurionCausalTickReceiptUnsignedV2,
  computeReceiptHash,
...
export class AuthoritativeMovementZone {
  ...
  restoreFromCanonicalState(
    ...
    for (const mob of state.mobs) {
      this.mobRuntime.restoreMob(mob.entityId, {
        x: mob.x,
        z: mob.z,
        health: mob.health,
        state: mob.state as any,
        targetEntityId: mob.targetEntityId,
        lastAttackTick: mob.lastAttackTick,
      });
    }
  ...
  tick(): boolean {
    ...
    // Phase 01: Membership and Revival
    let revivedCount = 0;
    ...
    const stage1State = this.getCanonicalZoneState();
    const stage1: AurionCausalStageReceipt = {
      stageOrdinal: 1,
      stageName: "MEMBERSHIP_REVIVAL",
      stageInputIdentity: canonicalSha256({ stage: "MEMBERSHIP_REVIVAL", tick: this.tickNumber, preStateHash }),
      canonicalStateHash: hashCanonicalZoneState(stage1State),
      transitionHash: canonicalSha256({ peersCount: this.peers.size, sortedCount: this.sortedPeers.length, revivedCount }),
    };
    ...
    const stage2State = this.getCanonicalZoneState();
    const stage2: AurionCausalStageReceipt = {
      stageOrdinal: 2,
      stageName: "MOVEMENT",
      stageInputIdentity: canonicalSha256(intentsToProcess.filter(i => i.type === "move").map(sanitizeIntentForHash)),
      canonicalStateHash: hashCanonicalZoneState(stage2State),
      transitionHash: canonicalSha256({ moved: changed, positions: this.sortedPeers.map(p => ({ id: `player:${p.userId}`, pos: p.position })) }),
    };
    ...
    const stage3State = this.getCanonicalZoneState();
    const stage3: AurionCausalStageReceipt = {
      stageOrdinal: 3,
      stageName: "PLAYER_ACTION",
      stageInputIdentity: canonicalSha256(intentsToProcess.filter(i => i.type !== "move").map(sanitizeIntentForHash)),
      canonicalStateHash: hashCanonicalZoneState(stage3State),
      transitionHash: canonicalSha256({ combatEvents: combatEvents.length, rngEvents: rngEvents.length, questSummaries: this.questSummaries.size }),
    };

    // Phase 04: Resource Transitions
    const resourcesChanged = this.resourceRuntime.tick(this.tickNumber);
    const stage4State = this.getCanonicalZoneState();
    const stage4: AurionCausalStageReceipt = {
      stageOrdinal: 4,
      stageName: "RESOURCE",
      stageInputIdentity: canonicalSha256({ stage: "RESOURCE", tick: this.tickNumber }),
      canonicalStateHash: hashCanonicalZoneState(stage4State),
      transitionHash: canonicalSha256({ resourcesChanged }),
    };

    // Phase 05: Mob FSM Transitions
    const mobsChanged = this.mobRuntime.tick(this.presences(), this.tickNumber);
    const stage5State = this.getCanonicalZoneState();
    const stage5: AurionCausalStageReceipt = {
      stageOrdinal: 5,
      stageName: "MOB_FSM",
      stageInputIdentity: canonicalSha256({ stage: "MOB_FSM", tick: this.tickNumber }),
      canonicalStateHash: hashCanonicalZoneState(stage5State),
      transitionHash: canonicalSha256({ mobsChanged }),
    };
    ...
    const stage6State = this.getCanonicalZoneState();
    const stage6: AurionCausalStageReceipt = {
      stageOrdinal: 6,
      stageName: "MOB_COMBAT",
      stageInputIdentity: canonicalSha256({ stage: "MOB_COMBAT", tick: this.tickNumber }),
      canonicalStateHash: hashCanonicalZoneState(stage6State),
      transitionHash: canonicalSha256({ mobCombatResult: mobCombatResult.changed, eventsCount: mobCombatResult.events.length }),
    };
    ...
    const stage7State = this.getCanonicalZoneState();
    const stage7: AurionCausalStageReceipt = {
      stageOrdinal: 7,
      stageName: "REGENERATION",
      stageInputIdentity: canonicalSha256({ stage: "REGENERATION", tick: this.tickNumber }),
      canonicalStateHash: hashCanonicalZoneState(stage7State),
      transitionHash: canonicalSha256({ staminas: this.sortedPeers.map(p => ({ id: `player:${p.userId}`, stamina: p.stamina })) }),
    };
    const stages: readonly AurionCausalStageReceipt[] = [stage1, stage2, stage3, stage4, stage5, stage6, stage7];
    ...
    const effectiveSchema = this.receiptSchemaOverride || AURION_CAUSAL_TICK_SCHEMA;
    const sourceRevision = this.sourceRevisionOverride || process.env.AURION_SOURCE_REVISION || activeProvenance.commit;

    let receipt: AurionCausalTickReceipt;
    if (effectiveSchema === AURION_CAUSAL_TICK_SCHEMA_V2) {
      const receiptUnsigned: AurionCausalTickReceiptUnsignedV2 = {
      ...
      const receiptUnsigned: AurionCausalTickReceiptUnsignedV1 = {
