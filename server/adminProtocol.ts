...
export function decodeValidatedGlbBase64(contentBase64: string): { bytes: Buffer; sha256: string } {
  if (!/^[A-Za-z0-9+/]+={0,2}$/.test(contentBase64) || contentBase64.length % 4 !== 0) {
    throw new Error("GLB payload is not canonical base64");
  }

  const bytes = Buffer.from(contentBase64, "base64");
  if (bytes.length < 12 || bytes.length > MAX_GLB_BYTES || bytes.toString("base64") !== contentBase64) {
