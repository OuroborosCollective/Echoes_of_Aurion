...
async function startServer(){
  ...
  // Apply global rate limiting to mitigate DoS and brute-force attacks
  app.use(rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 3000, // Limit each IP to 3000 requests per `window` (here, per 15 minutes) to allow normal game traffic while mitigating volumetric DoS
