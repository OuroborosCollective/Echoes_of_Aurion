...
suite("AIM-263 NPC packet from actual receipt storage",()=>{
 ...
 async function cleanup(){if(!isolated)throw Error("ISOLATED_TEST_DATABASE_REQUIRED");for(const table of ["aurionSemanticGraphIndexV2","aurionSemanticGraphProvenanceV2","aurionSemanticGraphEdgesV2","aurionSemanticGraphNodesV2","aurionSemanticGraphReceiptsV2"])await pool.query(`TRUNCATE TABLE ${table}`);await pool.query("TRUNCATE TABLE aurionNpcMemoryReceiptsV4");
