...
import { decodeOwnedNpcProjectionProvenance, projectOwnedNpcProjectionProvenance } from "../shared/npcSemanticGraphProvenanceReadmodel";
...
describe("AIM-292 WASD source and public transport boundary",()=>{
  it("binds the real imported rule functions and compiled identity to the reviewed WASD source",()=>{
    ...
    expect(appRouter._def.procedures).toHaveProperty("gameplay.npcProjectionProvenance");
  });
  ...
  it("derives projection provenance only from the bounded verified graph transport",()=>{
    const node={nodeId:"smn_"+"1".repeat(60),kind:"goal",semanticKey:"trade",status:"active",depth:1,score:1200,payloadHash:"2".repeat(64)};
    const graph={npcId:"lyra",generation:8,graphHash:"3".repeat(64),sourceResultHash:"4".repeat(64),resultHash:"5".repeat(64),sourceRevision:pin.sourceRevision,provenanceStatus:"VERIFIED",bounds:{maxDepth:4,maxCandidates:64,maxResults:32},nodes:[node],relations:[],excluded:{expired:0,contradicted:0,superseded:0}};
    const packet=projectOwnedNpcProjectionProvenance({userId:7,format:"aurion-public-npc-semantic-graph.v2",graphs:[graph]},7);
    expect(decodeOwnedNpcProjectionProvenance(packet,7).projections).toEqual([{npcId:"lyra",generation:8,graphHash:"3".repeat(64),sourceResultHash:"4".repeat(64),semanticGraphResultHash:"5".repeat(64),sourceRevision:pin.sourceRevision,provenanceStatus:"VERIFIED"}]);
    for(const changed of [
      {...packet,userId:8},
      {...packet,projections:[packet.projections[0],packet.projections[0]]},
      {...packet,projections:[{...packet.projections[0],provenanceHash:"private"}]},
      {...packet,projections:[{...packet.projections[0],memoryReceiptId:"private"}]},
