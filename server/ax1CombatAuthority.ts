...
export type HitResult = Readonly<{
  ...
  closestPointCapsule: Vec3;
  closestPointSegment: Vec3;
}>;
...
function finiteVec3(value: Vec3): void {
  if (![value.x, value.y, value.z].every(Number.isFinite))
    throw new Error("combat vector must be finite");
}
...
export function closestPointsBetweenSegments(
  s1: Segment3D,
  s2: Segment3D
): { p1: Vec3; p2: Vec3; distance: number } {
  finiteVec3(s1.p0);
  finiteVec3(s1.p1);
  finiteVec3(s2.p0);
  finiteVec3(s2.p1);
  ...
  if (a < epsilon && c < epsilon)
    return { p1: s1.p0, p2: s2.p0, distance: distance(s1.p0, s2.p0) };
  if (a < epsilon) {
...
export function testCapsuleAgainstSegment(
  capsule: Capsule3D,
  segment: Segment3D,
  nearMissMargin = 1.35
): HitResult {
  ...
  const closest = closestPointsBetweenSegments(
    { p0: capsule.base, p1: capsule.top },
    segment
  );
  ...
  return Object.freeze({
    ...
    isNearMiss:
      !hit && closest.distance <= capsule.radius + Math.max(0, nearMissMargin),
...
export class LagCompensationHistory {
  ...
  record(entityId: string, snapshot: CombatSnapshot): void {
    if (!entityId || !Number.isSafeInteger(snapshot.tick) || snapshot.tick < 0)
      throw new Error("invalid combat snapshot");
    const snapshots = this.history.get(entityId) ?? [];
    if (
      snapshots.length &&
      snapshot.tick <= snapshots[snapshots.length - 1]!.tick
    )
      throw new Error("combat snapshots must have monotone ticks");
    snapshots.push(
      Object.freeze({
        ...snapshot,
        position: Object.freeze({ ...snapshot.position }),
        capsule: Object.freeze({
          base: Object.freeze({ ...snapshot.capsule.base }),
          top: Object.freeze({ ...snapshot.capsule.top }),
  ...
  rewind(entityId: string, targetTick: number): CombatSnapshot | null {
    ...
    if (targetTick >= snapshots[snapshots.length - 1]!.tick)
    ...
    for (let index = 0; index < snapshots.length - 1; index += 1) {
      ...
      const alpha =
        (targetTick - left.tick) / Math.max(1, right.tick - left.tick);
      const position = lerp(left.position, right.position, alpha);
      const capsule = Object.freeze({
        base: lerp(left.capsule.base, right.capsule.base, alpha),
        top: lerp(left.capsule.top, right.capsule.top, alpha),
        radius: left.capsule.radius,
      });
      return Object.freeze({
        tick: targetTick,
        position,
        capsule,
        facingAngle:
          left.facingAngle + (right.facingAngle - left.facingAngle) * alpha,
      });
    }
    return snapshots[snapshots.length - 1]!;
  }
  ...
  validateSegmentHit(
    input: Readonly<{
      targetId: string;
      targetTick: number;
      attackSegment: Segment3D;
      maxRange: number;
      attackerPosition: Vec3;
    }>
  ): HitResult & { rewoundTick: number } {
    const state = this.rewind(input.targetId, input.targetTick);
    if (
      !state ||
      !Number.isFinite(input.maxRange) ||
      input.maxRange <= 0 ||
      distance(input.attackerPosition, state.position) > input.maxRange
    ) {
      return {
        hit: false,
        ...
        isNearMiss: false,
        penetrationDepth: 0,
        closestPointCapsule: input.attackerPosition,
        closestPointSegment: input.attackSegment.p0,
        rewoundTick: input.targetTick,
      };
    }
    return {
      ...testCapsuleAgainstSegment(state.capsule, input.attackSegment),
      rewoundTick: state.tick,
    };
  }
}
...
export type ReconciliationResult = Readonly<{
  ...
  corrected: boolean;
  replay: readonly PredictionSample[];
}>;
...
export class PredictionReconciler {
  ...
  push(sample: PredictionSample): void {
    if (!Number.isSafeInteger(sample.sequence) || sample.sequence <= 0)
      throw new Error("prediction sequence must be positive");
    if (
      this.pending.length &&
      sample.sequence <= this.pending[this.pending.length - 1]!.sequence
    )
      throw new Error("prediction sequence must be monotone");
    this.pending.push(
      Object.freeze({
  ...
  reconcile(
    acknowledgedSequence: number,
    authoritative: Readonly<{ x: number; z: number }>,
    predicted: Readonly<{ x: number; z: number }>
  ): ReconciliationResult {
    if (!Number.isSafeInteger(acknowledgedSequence) || acknowledgedSequence < 0)
      throw new Error("invalid server acknowledgement");
    this.pending = this.pending.filter(
      sample => sample.sequence > acknowledgedSequence
    );
    ...
    return Object.freeze({
      ...
      errorDistance,
      corrected: errorDistance > this.correctionThreshold,
      replay: Object.freeze(this.pending.slice()),
    });
  }
}
...
export class ThreatMatrix {
  ...
  add(mobId: string, actorId: string, amount: number): void {
    if (!mobId || !actorId || !Number.isFinite(amount) || amount < 0)
      throw new Error("invalid threat update");
  ...
  target(mobId: string): string | null {
    ...
    return Array.from(actors.entries()).sort(
...
export class EntityStateMachine {
  ...
  transition(next: EntityState): EntityState {
    ...
    const allowed: Record<EntityState, readonly EntityState[]> = {
      idle: ["patrol", "chase"],
      patrol: ["idle", "chase"],
      chase: ["attack", "retreat", "idle"],
      attack: ["chase", "retreat"],
      retreat: ["idle", "chase"],
      dead: [],
    };
    if (!allowed[this.state].includes(next))
      throw new Error(`invalid FSM transition ${this.state}->${next}`);
    this.state = next;
    return next;
  }
}
...
export class NavGrid {
  ...
  constructor(
    ...
    if (
      ...
      throw new Error("invalid nav grid");
  }
  setBlocked(point: GridPoint, blocked = true): void {
    blocked
      ? this.blocked.add(gridKey(point))
      : this.blocked.delete(gridKey(point));
  }
  private valid(point: GridPoint): boolean {
    return (
      point.x >= 0 &&
      point.z >= 0 &&
      point.x < this.width &&
      point.z < this.height &&
      !this.blocked.has(gridKey(point))
    );
  }
  findPath(start: GridPoint, goal: GridPoint): readonly GridPoint[] {
    ...
    const open = new Map<string, { point: GridPoint; g: number; f: number }>([
      [
        gridKey(start),
        {
          point: start,
          g: 0,
          f: Math.abs(goal.x - start.x) + Math.abs(goal.z - start.z),
        },
      ],
    ]);
    ...
    while (open.size) {
      // ⚡ Bolt: Replace O(N log N) Array.from().sort() with O(N) linear scan for A* performance
      let currentKey = "";
      let current: { point: GridPoint; g: number; f: number } | null = null;
      for (const [key, node] of open) {
        if (
          !current ||
          node.f < current.f ||
          (node.f === current.f && node.g < current.g) ||
          (node.f === current.f &&
            node.g === current.g &&
            key.localeCompare(currentKey) < 0)
        ) {
          current = node;
          currentKey = key;
        }
      ...
      if (!current) break; // Should never happen if open.size > 0
      if (currentKey === gridKey(goal)) {
        ...
        return Object.freeze(
          path.reverse().map(point => Object.freeze({ ...point }))
        );
      }
      closed.add(currentKey);
      const neighbours = [
        { x: current.point.x - 1, z: current.point.z },
        { x: current.point.x, z: current.point.z - 1 },
        { x: current.point.x, z: current.point.z + 1 },
        { x: current.point.x + 1, z: current.point.z },
      ];
      for (const neighbour of neighbours) {
        ...
        points.set(key, Object.freeze({ ...neighbour }));
        open.set(key, {
          point: neighbour,
          g,
          f:
            g + Math.abs(goal.x - neighbour.x) + Math.abs(goal.z - neighbour.z),
        });
      }
    }
    return [];
...
export type Buff = Readonly<{
  id: string;
  stat: "attack" | "defense" | "speed" | "regen";
  magnitudeBps: number;
...
export class BuffDebuffSystem {
  ...
  apply(buff: Buff, currentTick: number): void {
    if (
      !buff.id ||
      !Number.isSafeInteger(buff.magnitudeBps) ||
      !Number.isSafeInteger(buff.expiresAtTick) ||
      buff.expiresAtTick <= currentTick
    )
  ...
  multiplier(stat: Buff["stat"], tick: number): number {
    for (const [id, buff] of this.buffs)
      if (buff.expiresAtTick <= tick) this.buffs.delete(id);
    const totalBps = Array.from(this.buffs.values())
      .filter(buff => buff.stat === stat)
      .reduce((sum, buff) => sum + buff.magnitudeBps, 0);
...
export function advanceBallistic(
  ...
  if (!Number.isFinite(fixedDelta) || fixedDelta <= 0 || fixedDelta > 0.25)
    throw new Error("ballistic delta must be a bounded fixed step");
  const velocity = Object.freeze({
    x: state.velocity.x,
    y: state.velocity.y + gravity * fixedDelta,
    z: state.velocity.z,
  });
  const position = Object.freeze({
    x: state.position.x + velocity.x * fixedDelta,
    y: state.position.y + velocity.y * fixedDelta,
    z: state.position.z + velocity.z * fixedDelta,
  });
  return Object.freeze({ position, velocity });
}

export function hasLineOfSight(
  blockedSegments: readonly Segment3D[],
  ray: Segment3D,
  clearance = 0.05
): boolean {
  return !blockedSegments.some(
    blocker => closestPointsBetweenSegments(blocker, ray).distance <= clearance
  );
}

const commandDamage: Readonly<Record<string, number>> = Object.freeze({
  run: 0,
  interact: 0,
  attack: 17,
  skill_1: 15,
