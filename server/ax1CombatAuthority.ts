...
export class NavGrid {
  ...
  findPath(start: GridPoint, goal: GridPoint): readonly GridPoint[] {
    ...
    while (open.size) {
      let current: { point: GridPoint; g: number; f: number; key: string } | undefined = undefined;
      for (const node of open.values()) {
        if (!current) {
          current = node;
          continue;
        }
        if (node.f < current.f || (node.f === current.f && node.g < current.g) || (node.f === current.f && node.g === current.g && node.key.localeCompare(current.key) < 0)) {
          current = node;
        }
      }
