...
export class ThreatMatrix {
  ...
  target(mobId: string): string | null {
    ...
    // Optimized: O(N) linear scan to avoid array allocation and sorting overhead in high-frequency loop
    let bestActor: string | null = null;
    let maxThreat = Number.NEGATIVE_INFINITY;
    for (const [actorId, threat] of actors) {
      if (threat > maxThreat || (threat === maxThreat && bestActor !== null && actorId.localeCompare(bestActor) < 0)) {
        maxThreat = threat;
        bestActor = actorId;
      }
    }
