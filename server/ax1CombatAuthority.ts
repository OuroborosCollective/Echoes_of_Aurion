...
export class ThreatMatrix {
  ...
  target(mobId: string): string | null {
    ...
    for (const [actorId, threat] of actors.entries()) {
      if (threat > maxThreat || (threat === maxThreat && (topActor === null || actorId.localeCompare(topActor) < 0))) {
        topActor = actorId;
        maxThreat = threat;
      }
    }
  ...
  findPath(start: GridPoint, goal: GridPoint): readonly GridPoint[] {
    ...
    while (open.size) {
      ...
      for (const node of open.values()) {
        if (!current || node.f < current.f || (node.f === current.f && (node.g < current.g || (node.g === current.g && node.key.localeCompare(current.key) < 0)))) {
          current = node;
        }
      }
      if (!current) break;
