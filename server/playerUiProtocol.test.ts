...
describe("AX1 controls and confirmed paperdoll contract", () => {
  it("accepts five distinct server commands and rejects invented, repeated or authority-bearing settings", () => {
    const settings = { revision: 0, autoLoot: true, analyticsConsent: false, hotbar: defaultHotbar, movementMode: "joystick" as const };
  ...
  it("accepts only the two supported mobile movement owners", () => {
    const base = { revision: 0, autoLoot: true, analyticsConsent: false, hotbar: defaultHotbar, movementMode: "joystick" as const };
    expect(controlSettingsSchema.parse(base).movementMode).toBe("joystick");
    expect(controlSettingsSchema.parse({ ...base, movementMode: "touch_to_move" }).movementMode).toBe("touch_to_move");
    expect(controlSettingsSchema.safeParse({ ...base, movementMode: "radial" }).success).toBe(false);
  });
