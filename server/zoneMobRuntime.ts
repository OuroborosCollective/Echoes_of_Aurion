...
export class ZoneMobRuntime {
  ...
  restoreMob(
    entityId: string,
    values: { x: number; z: number; health: number; state: import("../shared/zoneMobContract").ZoneMobState; targetEntityId: string | null; lastAttackTick?: number }
  ): MobRuntimeState | undefined {
    const current = this.states.get(entityId);
    if (!current) return undefined;
    const next = restoreMobRuntimeState(current, values);
    this.states.set(entityId, next);
    return next;
  }
