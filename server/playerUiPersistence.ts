...
export async function readControlSettings(tx: Database | UiTransaction, userId: number): Promise<ControlSettings> {
  ...
  if (row && ((row.autoLoot !== 0 && row.autoLoot !== 1) || (row.analyticsConsent !== 0 && row.analyticsConsent !== 1) || !["joystick", "touch_to_move"].includes(row.movementMode))) throw new Error("UI_SETTINGS_CORRUPT");
  return row ? controlSettingsSchema.parse({ revision: row.revision, autoLoot: row.autoLoot === 1, analyticsConsent: row.analyticsConsent === 1, hotbar: JSON.parse(row.hotbarJson), movementMode: row.movementMode }) : { revision: 0, autoLoot: true, analyticsConsent: false, hotbar: [...defaultHotbar], movementMode: "joystick" };
}
...
export async function savePlayerControls(userId: number, expected: ControlSettings) {
  ...
  return db.transaction(async tx => {
    ...
    const row = { userId, revision: input.revision + 1, autoLoot: input.autoLoot ? 1 : 0, analyticsConsent: input.analyticsConsent ? 1 : 0, hotbarJson: JSON.stringify(input.hotbar), movementMode: input.movementMode };
