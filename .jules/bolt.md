...
## 2026-09-08 - Array Allocation Overhead in Zone Tick Loop
**Learning:** Found a severe performance bottleneck in `server/zoneMobRuntime.ts` where methods called in the 10Hz game loop (`tick()`, `orderedStates()`, `snapshot()`) were allocating excessive intermediate arrays using `Array.from(...)` and `.map(...)` alongside Map `.get()` lookups. In a high-frequency tick environment, these temporary allocations create significant garbage collection pressure and CPU overhead which can impact server latency.
**Action:** Replaced dynamic array creations and Map usage with static parallel array caches (e.g. `this.cachedOrderedStates`) populated at construction, coupled with an `entityIdToIndex` mapping. Operations inside the game loop were converted to use pre-allocated arrays and `for` loops with index access.
