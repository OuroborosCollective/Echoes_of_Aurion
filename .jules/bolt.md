...
## 2026-09-15 - Optimize Game Loop with Parallel Arrays
**Learning:** In high-frequency game loops (like the 100ms ticks in `zoneMobRuntime.ts`), looking up entities by string ID using `Map.get()` on every tick can cause garbage collection latency and map hashing overhead. V8 handles static array iteration much more efficiently.
**Action:** Replace map lookups inside the hot path with parallel cached arrays. Maintain a `Map<string, number>` to map dynamic keys to static array indices, and execute all loop iterations over the contiguous array.
