...
## 2026-09-22 - O(1) Cached Array for Zone Mob Runtime States
**Learning:** In high-frequency server game loops (like `server/zoneMobRuntime.ts`), using `Map.get()` and `.map()` to locate entities or create snapshot arrays on every tick causes unnecessary CPU overhead and GC latency.
**Action:** Implemented a parallel cached array containing pre-looked up objects, keeping them synchronized during state mutations via an `entityIdToIndex` lookup map. This replaces (N)$ map iterations and temporary array allocations with (1)$ direct array access, reducing GC pressure and execution time.
