...
## 2024-11-20 - High-Frequency A* Pathfinding and Threat Matrix Optimization
**Learning:** Found significant garbage collection overhead in `NavGrid.findPath` and `ThreatMatrix.target` within `server/ax1CombatAuthority.ts` caused by calling `Array.from(...).sort()` on Maps to find single minimum/maximum values inside hot loops. String creation via `gridKey()` inside the `NavGrid` loop also caused allocation overhead.
**Action:** Replaced $O(N \log N)$ `Array.from().sort()[0]` patterns with linear $O(N)$ `for...of` loops to track min/max values without allocating intermediate arrays. Pre-cached the grid keys directly in the Map value objects for A* open nodes to avoid generating and comparing strings dynamically during the iterations.
