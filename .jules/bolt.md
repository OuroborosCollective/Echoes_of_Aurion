...
## 2024-11-20 - Redundant Array Allocation and Sorting in getCanonicalZoneState
**Learning:** Discovered a performance bottleneck where `getCanonicalZoneState` in `server/zoneRuntime.ts` was doing redundant sorting of maps (`Array.from(peer.skillCooldownUntilTick.entries()).sort(...)`) and redundant map mapping (`Array.from(this.questSummaries.values())`) inside a high-frequency loop. `sortCanonicalZoneState` already handles sorting of fields downstream.
**Action:** Replace `Array.from(...).sort(...)` with a simple Map iteration (`for...of`) when the final object is already sorted later, and use explicit caching/dirty flags (e.g. `cachedQuestSummaries` updated via `this.questSummariesDirty`) instead of mapping map values dynamically on every tick.
