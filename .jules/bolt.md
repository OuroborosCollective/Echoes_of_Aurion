...
## 2024-05-24 - Optimizing map sorts in high-frequency game loops
**Learning:** In high-frequency game loops (like tick functions), calling `Array.from(map.entries()).sort().map()` creates unnecessary intermediate tuple arrays for the entries and adds overhead with the extra `.map()` iteration, putting pressure on the garbage collector.
**Action:** When you only need the values, use `Array.from(map.values()).sort(...)` instead. This directly creates an array of values, avoiding intermediate tuple allocation and the extra iteration step.
