...
## 2024-05-24 - Array allocation and Map lookups in high-frequency game loop
**Learning:** In `server/zoneMobRuntime.ts`, iterating over O(N) array items and looking up each state using O(1) `Map.get()` caused excess garbage collection latency.
**Action:** Always avoid Map lookups and dynamic array allocations in hot paths like game ticks or network snapshots. Instead, construct cached arrays (e.g., `cachedOrderedStates`) and index mapping structures in the constructor for static keys, allowing zero-allocation $O(1)$ loop access directly to state objects.
