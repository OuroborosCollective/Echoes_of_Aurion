...
## 2024-06-25 - [Optimize BuffDebuffSystem multiplier]
**Learning:** High-frequency game loops (like combat authority multiplier checks) shouldn't use chained array methods (like `Array.from(map.values()).filter().reduce()`) as they introduce unnecessary string/array generation and higher-order iteration overhead.
**Action:** Use a single `for...of` linear scan over the map's entries to simultaneously clear expired entries and aggregate values in $O(N)$ time, avoiding GC spikes.
