...
## 2024-09-15 - Add Explanatory Tooltips and Disabled Styles to Community Forms
**Learning:** Native `<button>` elements in standard HTML forms lack context when disabled. In community or forum features, it is a common pattern to disable submit buttons until the user fills in required inputs. However, without a `title` attribute or a `disabled:cursor-not-allowed` class, users on desktop (or using screen readers) might be confused as to why the form is unresponsive.
**Action:** Always pair `disabled` form submission buttons with `disabled:cursor-not-allowed` for visual feedback, and use a `title` attribute (e.g., `title={!isValid ? "Bitte gib eine Nachricht ein" : undefined}`) to provide a descriptive explanation for screen reader users and hover interactions, explaining *why* the state is disabled.
