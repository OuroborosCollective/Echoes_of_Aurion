...
## 2024-09-17 - Button Disabled & Loading States
**Learning:** When using loading states that act as a disabled affordance, buttons should include `aria-busy={loading}` along with `disabled={loading}`. Furthermore, interactive buttons utilizing tactile feedback (`motion-safe:hover:-translate-y-0.5`, `motion-safe:active:scale-95`) must also reset these transforms using `disabled:motion-safe:hover:translate-y-0` and `disabled:motion-safe:active:scale-100` alongside standard `disabled:opacity-60 disabled:cursor-not-allowed` styles to prevent unintended visual transitions when disabled. Providing a contextual `title` text for loading is also important for accessibility.
**Action:** When adding or modifying interactive buttons that have a disabled or loading state, explicitly define `aria-busy`, contextual `title` text, and reset any active transform/scale utility classes with `disabled:` modifiers to ensure a robust, accessible UX.
