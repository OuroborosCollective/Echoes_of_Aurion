...
The local isolated suite initially passed 17 tests; twelve faults were detected and the absent attestation correctly remained UNPROVABLE. Exact technical head `2fc1edb255abb93cc1fd75784ebe0494065bde3e` then passed workflow `35536870565`, job `106147355220`: 5 files / 39 tests, all thirteen expected boundaries detected, real GitHub OIDC/Sigstore signing, original-byte verification, one-byte tamper rejection, restored-byte verification, and an immutable report artifact. The report binds `testedRevision` to the branch head and preserves the attestation's separate workflow/ref identity; neither is relabelled as production evidence. Local Test Pack, runtime candidate/container and Android also passed on that exact technical head. The Memory append creates a new final head, so final-head checks, predecessor integration, merge and production readback remain required. This draft depends on Step 29 (#433) and will be promoted only after its predecessor is merged and read back.
