...
export type MerchantActionSourceDecision = Readonly<{
    receiptId: string;
...
export type MerchantActionEpoch = Readonly<{
    npcResolutionIndex: number;
    marketVersion: number;
    polityVersion: number;
}>;
export type MerchantMarketEvidence = Readonly<{
    version: number;
    stateHash: string;
}>;
/** Full target evidence is supplied; the gateway derives a canonical target hash. */
export type MerchantActionTargetEvidence = Readonly<{
    id: string;
    kind: "market";
    market: MarketState;
    version: number;
...
export type MerchantActionTarget = Readonly<{
    id: string;
    kind: "market";
    hubId: HubId;
    version: number;
...
export type MerchantInventoryEntry = Readonly<{
    itemId: CommodityId;
    quantity: number;
    capacity: number;
}>;
export type MerchantInventoryEvidence = Readonly<{
    ownerId: string;
    stateHash: string;
    entries: readonly MerchantInventoryEntry[];
}>;
export type MerchantPolityEvidence = Readonly<{
    polityId: string;
    version: number;
    stability: number;
    stateHash: string;
...
export type MerchantActionReady = Readonly<{
    status: "ready";
    intent: MerchantActionIntent;
    proposedLease: MerchantActionLease;
    resolution: LivingWorldResolution;
}>;
export type MerchantActionValidated = Readonly<{
    status: "validated";
    intent: MerchantActionIntent;
    lease: MerchantActionLease;
    receipt: MerchantActionReceipt;
    resolution: LivingWorldResolution;
    requests: MerchantDecisionRequests;
}>;
...
export declare function merchantInventoryStateHash(input: Readonly<{
    ownerId: string;
    market: MarketState;
    entries: readonly MerchantInventoryEntry[];
}>): string;
export declare function merchantPolityStateHash(input: Readonly<{
    polityId: string;
    version: number;
    stability: number;
}>): string;
...
export declare function validateMerchantAction(input: Readonly<{
    context: MerchantGatewayContext;
    intent: MerchantActionIntent;
    lease: MerchantActionLease;
}>): MerchantActionValidated | MerchantActionBlocked;
