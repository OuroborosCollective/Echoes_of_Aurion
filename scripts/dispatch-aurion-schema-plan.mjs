...
export function validateSchemaDispatchPlan({ manifest, expectedSha, planSha256, ledgerRunId }) {
  ...
  if (manifest?.waveId !== "aurion-production-0021-0050" || manifest.schemaVersion !== "aurion.migration-wave-manifest.v2" || manifest.policy?.ownerApprovalRequired !== true || manifest.policy?.productionWritesScheduled !== false || manifest.migrations?.map(entry => entry.tag).join(",") !== tags.join(",")) throw new Error("SCHEMA_DISPATCH_WAVE_NOT_AUTHORIZED");
}
