...
try {
  ...
  await page.getByRole("heading", { name: /Eine Welt, die nicht auf dich wartet\./i }).waitFor({ state: "visible", timeout: 60_000 });
  await page.getByRole("link", { name: "GAMEPLAY ENTDECKEN", exact: true }).waitFor({ state: "visible", timeout: 60_000 });
  if (await page.locator("canvas").count()) throw new Error("Aurion portal must not mount a gameplay canvas.");
  if (await page.getByText(/ARENA 1\/4|WELT \/ QUESTS|AX1 OPEN WORLD STARTEN|LOADOUT VORBEREITEN/i).count()) throw new Error("Legacy gameplay chrome is visible on the Aurion landing page.");
  ...
  console.log(`android_social_keyframe_captured path=${outputPath} viewport=1200x630 source=aurion-living-world readonly=true gameplayCanvas=false`);
} finally {
  await browser.close();
}
