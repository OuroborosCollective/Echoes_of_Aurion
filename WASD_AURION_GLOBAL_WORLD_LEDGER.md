...
## Fortschreibung: Mehrchunkfenster und gebundene Umwelt-/Gesellschaftsepochen

> **Gebundene Ausgangslage:** WASD `a4d99432e47b82ce98105eadb30360cd8040ad13`; Aurion-Kandidatenstand vor diesem Schnitt `c607c7dbca912f99fc33b30f742b6291b26f6ca4` auf `aurion/wasd-unification-continuation`. Diese Fortschreibung bleibt additiv. Die Migration `0024_aurion_world_epoch_reactions.sql` wurde **nicht** ausgeführt; `main`, VPS-Dienste und Produktionsdaten blieben unverändert.

| Vertragsbereich | Realisierung und Nachweis | Status |
| --- | --- | --- |
| Mehrchunkfenster | `shared/worldChunkStreamingProtocol.ts` plant für Phone/Tablet/Desktop stabil center-first sichtbare Chunks, fehlende Requests und LRU-Entsorgung. Sichtbare Chunks bleiben gepinnt; Budgets sind 12/20/36 Chunks bei Radien 1/1/2. `gameplay.worldChunkWindow` akzeptiert nur festen Tier- und Integer-Zentruminput, liefert je Chunk höchstens 32 Deltas und bindet Cursor exakt an das sichtbare Fenster. | Regel-, Readmodel- und Routentests bestanden |
| Browser-/Babylonadapter | `Home.tsx` leitet den festen Darstellungs-Tier aus dem Viewport ab und fragt ausschließlich geschützte Window-Readmodels an. `scene.ts` hält bestätigte Chunkwurzeln in einem LRU-Cache, vereinigt maximal 32-Delta-Seiten per Receipt-ID und entsorgt nur vom Shared-Plan freigegebene Hintergründe. Renderer, Browser und Client erhalten weder Seedautorität noch frei wählbare Sichtweiten. | Typ- und Komponententests bestanden; GPU-Canvas-Readback offen |
| Epochreaction-Vertrag | `worldEpochReactionProtocol.ts` ersetzt jeden Wasd-Tick durch einen reinen, versionierten Schritt, dessen `resolutionIndex` exakt der bestätigten Weltepoche entspricht. Höchstens 48 Sektoren, 192 Deltas und 128 serverbeobachtete Präsenzen werden pro Auflösung stabil sortiert verarbeitet. Der Receipt enthält Quellen, Regel-/Contentversion, Hash und begrenzte Resultate. | Unit-Determinismus- und Grenztests bestanden |
| Ökologie und Ökonomie | Bestätigte Baum-Entnahmen, Straßen und erlaubte Strukturen beeinflussen nur die zugeordneten Sektoren: Waldgesundheit, Wiederbewuchs, Dürre, Wasser, Nahrung und Erz werden als deterministische Bilanz fortgeschrieben. Daraus folgen Marktbestände/-preise für Holz, Moonwheat und Asterion-Eisen sowie bedarfsgebundene Farmer-, Forster-, Händler-, Bau- und Kräuterkundigenzahlen. | Reiner Vertrag und Snapshotreadback getestet |
| Migration, Politik und Quests | Dürre, Waldschäden und fiktiver Konfliktdruck erzeugen ausschließlich geschützte Migrationsrouten und data-only Questangebote. Zivilstrukturen und Spielerhäuser bleiben explizit geschützt; Infrastruktur, Präsenz, Nahrung und Wasser tragen nur zu einer deeskalierenden Politikkomponente bei. Es gibt weder reale Gewaltlogik noch automatische Belohnungs-, Loot- oder Questabschlussautorität. | Reiner Vertrag und Schutzassertionen getestet |
| Atomare Persistenz und Spielerreadback | Die bestehende explizite Admin-Epochtransaktion liest kanonisch begrenzte bestätigte Deltas, berechnet die Reaction und persistiert sie zusammen mit dem Globalsnapshot als eindeutiges `worldId`/`epoch`-Receipt. `getOpenWorldSnapshot()` liest eine Reaction nur bei passendem Welt-, Epochen-, Versions- und Snapshot-Hash; sonst bleibt sie wirkungslos. | Typ-/Snapshottests bestanden; physischer DB-Readback offen |
| Datenbanknachweis | `server/worldEpochReaction.e2e.test.ts` prüft opt-in Replay, atomare Reactionbindung und konkurrierende Epochen. Sie läuft ausschließlich bei `NODE_ENV=test`, `DATABASE_URL` und `AURION_WORLD_EPOCH_E2E=1`; im aktuellen Lauf wurden **zwei Fälle erwartungsgemäß übersprungen**. | offen bis isolierte Testdatenbank aktiviert ist |

Der vollständige lokale Lauf nach diesem Schnitt ergab **53 bestandene** und **9 erwartungsgemäß übersprungene Testdateien** sowie **181 bestandene** und **17 übersprungene Tests**. `pnpm check`, `git diff --check` und die semantischen Diagnosen von `worldEpochReactionProtocol.ts`, `db.ts` und `openWorldProtocol.ts` waren erfolgreich. Die lokalen Browserversuche nutzten zunächst einen separaten Vite-Prozess ohne API-Backend und lieferten deshalb erwartbar `404`/`500`; dieser Prozess wurde beendet, der integrierte Health-Endpoint auf Port 3000 antwortet. Da kein authentifizierter DB-Readmodelpfad und kein erfolgreicher Chromium-WebGL-Canvasreadback vorliegt, wird keine sichtbare Mehrchunkabnahme behauptet.
