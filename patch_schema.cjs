...
export const aurionCivilizationHistoryEvents = mysqlTable("aurionCivilizationHistoryEvents", {
  ...
  return {
    historySequenceIdx: uniqueIndex("historySequenceIdx").on(table.worldId, table.civilizationId, table.occurredSequence),
  };
});
...
`;
if (!content.includes('aurionCivilizationHistoryEvents')) {
