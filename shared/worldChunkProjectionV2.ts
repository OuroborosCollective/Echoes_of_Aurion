...
export const WORLD_CHUNK_PROJECTION_VERSION_V2 = WORLD_CHUNK_PROJECTION_V2_VERSION;
...
export async function createWorldChunkProjectionManifestV2(
  input: Omit<WorldChunkProjectionManifestV2, "manifestHash" | "projectionHash">
  ...
  const projectionHash = `sha256:${hex}`;
  const manifestIdentity = { ...identity, projectionHash };
  const manifestSerialized = JSON.stringify(manifestIdentity, Object.keys(manifestIdentity).sort());
  const manifestHex = sha256Bytes(new TextEncoder().encode(manifestSerialized));
