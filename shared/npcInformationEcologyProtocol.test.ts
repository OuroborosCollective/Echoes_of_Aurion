...
describe("AIM-487 deterministic NPC information ecology", () => {
  ...
  it("creates deterministic communication identity and preserves origin evidence", () => {
    ...
    expect(a.sourceReceiptId).toBe(source.sourceReceiptId);
  ...
  it("keeps provenance available for compact readmodels", () => {
    ...
    expect(summary.originSourceReceiptId).toBe(source.sourceReceiptId);
  ...
  it("only treats corroborated/trusted non-expired facts as consumable", () => {
    ...
    const trusted = transitionNpcInformation(remembered, { status: "trusted", logicalIndex: 12, confidenceBps: 9000 });
    expect(isVerifiedConsumableNpcInformation(trusted, 12)).toBe(true);
