 ...
 * - Finite numbers preserve exact values, normalizing negative zero to "0".
...
export function canonicalJson(obj: unknown): string {
  ...
  if (typeof obj === "number") {
    if (!Number.isFinite(obj)) throw new Error("CANONICAL_NUMBER_NON_FINITE");
...
/**
 * Pure deterministic standard SHA-256 implementation (FIPS 180-4).
 * Runs universally in Node.js, Worker, and Browser environments without platform dependencies.
 */
export function sha256Bytes(bytes: Uint8Array): string {
  ...
  view.setBigUint64(newLength - 8, BigInt(bitLength));
