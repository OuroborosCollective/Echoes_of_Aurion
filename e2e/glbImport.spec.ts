...
test("admin upload persists bytes and assignment, deduplicates, scrolls on mobile, and renders the published avatar", async ({ page, baseURL }, testInfo) => {
  ...
  try {
    ...
    await expect.poll(() => scrollRegion.evaluate(element => Math.round(element.scrollTop)), { timeout: 10_000 }).toBeGreaterThan(0);
    await scrollRegion.press('Home');
    await expect.poll(() => scrollRegion.evaluate(element => Math.round(element.scrollTop)), { timeout: 10_000 }).toBe(0);
