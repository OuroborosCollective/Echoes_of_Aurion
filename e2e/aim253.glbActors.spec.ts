...
for (const viewport of [
  ...
  test(`real GLB actors upload, animate, ground and open the smith on ${viewport.name}`, async ({ page, baseURL }, testInfo) => {
    ...
    try {
      ...
      await expect.poll(async () => (await pose(player))?.bonePose, { timeout: 15_000 }).not.toBe(idle.bonePose);
      ...
      await expect.poll(async () => (await pose(smith))?.bonePose, { timeout: 15_000 }).not.toBe(initialSmith.bonePose);
